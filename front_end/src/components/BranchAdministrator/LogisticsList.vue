  ...
  <div>
    <div>
      ...
      <div id="search">
        ...
        <div id="searchContent">
          <el-input  v-model="begindate" placeholder="请输入开始日期"></el-input>
        </div>

        <div id="searchContent">
          <el-input  v-model="endate" placeholder="请输入结束日期"></el-input>
        </div>
  ...
  methods: {
    ...
    searchByfactor(pageNum){
      ...
      this.$axios({
        ...
        url: 'http://8.130.39.140:8081/express/api/network-administrator/getExpressByStateAndTime/?networkId='+this.networkId+'&page='+pageNum+'&pageSize='+this.pagesize+'&state='+this.value+'&dateStart='+this.begindate+'&dateOver='+this.endate,
    ...
    querySearch(pageNum) {
      this.$axios({
        ...
        // data: JSON.stringify(info),
        url: 'http://8.130.39.140:8081//express/api/network-administrator/getExpress/?networkId=1'+'&page='+pageNum+'&pageSize='+this.pagesize,
      }).then((response) => {
//这里使用了ES6的语法
