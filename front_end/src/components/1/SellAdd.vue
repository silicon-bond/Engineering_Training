    ...
    <div id="messageBox">
      ...
      <div id="message">
        <el-form :model="ruleForm" :inline="true" :label-position="labelPosition" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item style="width: 40%" label="价格(万元/套)" label-width="100" prop="price">
      ...
      this.$refs[message].validate((valid) => {
        if (valid) {
          this.$axios({
            ...
            url: 'http://localhost:8081/javaee1_war_exploded/sell/insert',
          }).then((response) => {          //这里使用了ES6的语法
            if (response.data.code === '200'){
              alert('添加成功');
              this.$router.push('/user/housesell')
            } else{
              alert('添加失败');
            }
          }).catch((error) => {
